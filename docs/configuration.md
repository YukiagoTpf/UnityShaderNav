...
gated behind a Unity variant keyword from an exact `multi_compile` /
`shader_feature` declaration (built-in shortcut pragmas are not expanded). Definitely inactive branches use plain fading; variant-dependent
